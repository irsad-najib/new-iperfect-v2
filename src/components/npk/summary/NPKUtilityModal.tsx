...
interface NPKInputOutputModalProps {
  open: boolean;
  onCancel: () => void;
...
const NPKUtilityModal = ({
  ...
  const [availableInputs, setAvailableInputs] = useState<unknown[]>([]);
      ...
      const errorMessage =
        ...
        (error as { response?: { data?: { message?: string } } }).response?.data
          ...
          : error instanceof Error
            ? error.message
            : "Unknown error";
      message.error(`Failed to create UDF: ${errorMessage}`);
    } finally {
  ...
  const saveUdfChanges = useCallback(async (): Promise<{
    ...
    try {
      ...
      // Call edit utility cell endpoint to update the cell with new/updated UDF
      if (configId && cellKey) {
        // Derive unit and API material key from material name
        const derivedUnit = getUnitFromMaterialName(materialName);
        if (derivedUnit && !unit) setUnit(derivedUnit);
        const materialKey = getMaterialKeyFromName(materialName);

        await api.post("/npk/daily/summary/utility/edit", {
          config_id: configId,
          cell_key: cellKey,
          udf_id: finalUdfId,
      ...
      const errorMessage =
        ...
        (error as { response?: { data?: { message?: string } } }).response?.data
          ...
          : error instanceof Error
            ? error.message
            : "Unknown error";
      message.error(`Failed to save or add UDF: ${errorMessage}`);
      return { success: false };
    } finally {
  ...
  return (
    <>
      <Modal
        ...
        centered
        maskClosable={false}
        wrapClassName="[&_.ant-modal-content]:rounded-lg [&_.ant-modal-header]:border-b-0 [&_.ant-modal-header]:px-6 [&_.ant-modal-header]:py-4 [&_.ant-modal-body]:px-6 [&_.ant-modal-body]:pb-6 [&_.ant-modal-body]:pt-0">
        <div className="flex flex-col gap-4">
          ...
          <div>
            ...
            />
            <div className="flex justify-between py-2 pr-[70px] text-neutral-900 text-14 rounded-t-md">
              <span>Template name</span>
              <span>Last saved</span>
            </div>
            <List
              className="h-60 overflow-y-auto rounded-lg bg-neutral-100 [&_.ant-list-item]:px-4! [&_.ant-list-item]:py-[11px]! [&_.ant-list-item]:cursor-pointer [&_.ant-list-item]:flex [&_.ant-list-item]:items-center [&_.ant-list-item]:gap-4 [&_.ant-list-item:hover]:bg-neutral-200"
              ...
              renderItem={(item) => (
                <List.Item
                  ...
                  <div
                    className={`text-16 flex items-center gap-2 ${
                      selectedTemplate === item.udf_id
                        ? "text-neutral-100"
                        : "text-neutral-900"
                    }`}>
          ...
          <>
            <div className="flex flex-col items-start gap-[18px] mt-7">
              <div className="flex items-center justify-between w-full">
                <div>
                  <span className="text-neutral-900 text-16 font-semibold">
                    Material Name: {""}
                  </span>
                  <span className="bg-neutral-250 py-1 px-3 ml-[9px] mr-[18px] rounded-md text-neutral-900 text-16 font-normal">
                    {materialName}
                  </span>
                  <span className="text-neutral-900 text-16 font-semibold">
                    Unit:{""}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-col gap-3">
                  <div className="flex justify-between items-center">
                    <h3 className="text-16 font-semibold m-0 text-neutral-900">
                      Inputs
                    </h3>
                    <Button
                  ...
                  <div className="bg-white border border-neutral-300 rounded-lg p-4 h-[600px] max-h-max overflow-hidden">
                    <div className="flex flex-col gap-0.5 h-full overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-neutral-100 [&::-webkit-scrollbar-track]:rounded [&::-webkit-scrollbar-thumb]:bg-neutral-300 [&::-webkit-scrollbar-thumb]:rounded [&::-webkit-scrollbar-thumb:hover]:bg-neutral-500">
                      <div className="grid grid-cols-[150px_150px_minmax(0,2fr)_70px_120px_70px] gap-1.5 px-0 pb-2.5 sticky top-0 bg-white z-10">
                        {[
                          "Nama",
                          "Data Source",
                          "Data Path",
                          ...
                          <span
                            ...
                            className="text-14 font-semibold text-neutral-900 text-center whitespace-nowrap overflow-hidden text-ellipsis px-2">
                            {label}
                          </span>
                        ))}
                      </div>
                      {udfData?.inputs?.map((input, index) => (
                        <div
                          ...
                          <span
                            ...
                            className="text-14 text-neutral-100 font-semibold block p-2 min-h-[38px] border border-secondary-300 rounded-md whitespace-nowrap overflow-hidden text-ellipsis text-center leading-[22px] bg-secondary-300">
                            {input.var_name}
                          </span>
                          <span
                          ...
                          <span
                            ...
                            className="text-14 text-neutral-900 font-normal block p-2 min-h-[38px] border border-neutral-300 rounded-md whitespace-nowrap overflow-hidden text-ellipsis text-center leading-[22px] [direction:rtl]">
                            {input.ref_name}
                          </span>
                          ...
                          <div className="flex items-center justify-center min-w-[70px] border-none p-0">
                            <MdEditNote
                              size={28}
              ...
              <div className="flex-1 min-w-0 flex flex-col">
                ...
                {activeTestTab === "test" && (
                  <div className="flex flex-col gap-2 rounded-lg grow py-1">
                    <div className="flex justify-start items-start bg-neutral-200 rounded-md h-152">
                      <div className="text-14 text-neutral-900 font-medium p-4 flex items-start justify-start h-full w-full bg-neutral-200 rounded-md">
                ...
                {activeTestTab === "debug" && (
                  <div className="flex flex-col gap-2 rounded-lg grow py-1">
                    <div className="rounded overflow-hidden">
                      <div className="bg-neutral-250 py-2 px-4 text-16 font-semibold text-center">
                        STDOUT
                      </div>
                      <div className="bg-neutral-200 p-4 h-144 overflow-auto">
                        <div className="text-neutral-700 text-16 text-left whitespace-pre-wrap">
                          {testResult !== null
                            ? `${testResult?.std_out}\nlog:\n${JSON.stringify(
                                testResult.log,
                                null,
...
export default NPKUtilityModal;
